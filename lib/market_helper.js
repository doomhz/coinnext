(function() {
  var AVAILABLE_MARKETS, CURRENCIES, CURRENCY_NAMES, FEE, MARKET_STATUS, MarketHelper, ORDER_ACTIONS, ORDER_STATUS, ORDER_TYPES, PAYMENT_STATUS, TOKENS, TRANSACTION_ACCEPTED_CATEGORIES, TRANSACTION_MIN_CONF, WITHDRAWAL_FEES, exports, math, _;
  ...
  MarketHelper = {
    ...
    calculateResultAmount: function(amount, action, unitPrice) {
      ...
      return math.multiply(parseFloat(amount), parseFloat(unitPrice));
    },
    calculateFee: function(amount) {
      return math.select(parseFloat(amount)).divide(100).multiply(this.getTradeFee()).done();
    },
