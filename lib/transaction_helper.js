  ...
  TransactionHelper = {
    ...
    processPayment: function(payment, callback) {
      return MarketStats.findEnabledMarket(payment.currency, "BTC", function(err, market) {
        if (!market) {
          return next(new restify.ConflictError("" + (new Date()) + " - Will not process payment " + payment.id + ", the market for " + payment.currency + " is disabled."));
        }
    ...
    loadTransaction: function(transactionOrId, currency, callback) {
      var txId;
      ...
      return MarketStats.findEnabledMarket(currency, "BTC", function(err, market) {
        if (!market) {
          return next(new restify.ConflictError("" + (new Date()) + " - Will not load the transaction " + txId + ", the market for " + currency + " is disabled."));
        }
