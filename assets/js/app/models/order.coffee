  ...
  calculateSecondAmount: ()->
    if @get("status") is "partiallyCompleted"
      if @get("type") is "market"
        ...
        return App.math.multiply App.add(@get("amount"), -@get("sold_amount")), @get("unit_price")  if @get("action") is "sell"
      if @get("type") is "limit"
        return App.math.multiply App.add(@get("amount"), -@get("result_amount")), @get("unit_price")  if @get("action") is "buy"
        return App.math.multiply App.add(@get("amount"), -@get("sold_amount")), @get("unit_price")  if @get("action") is "sell"
    if @get("status") is "completed"
